...
```{r lucas-raimondi, fig.cap="(ref:lucas-raimondi)"}
...
```{r goltzius-wedding, fig.cap="(ref:goltzius-wedding)"}
...
par(mar = c(1, 3, 1, 1))
plot(x)
barplot(c(a_eii, b_eii, c_eii), ylab = "EI Index", xlab = "Group", names.arg = c("A", "B", "C"), col = pal, ylim = c(-1, 1))
```
...
```{r europe-pop, fig.cap = "(ref:europe-pop)", fig.fullwidth = TRUE}
region_map <- c(
...
```{r rkm-flem-ind-eii, fig.fullwidth = TRUE, fig.cap="The individual EI scores of highly-central Flemish printmakers and publishers in 1560, 1585, 1615, 1645, 1675, 1715"}
...
```{r rkm-dutch-ind-eii, fig.fullwidth = TRUE, fig.cap="The individual EI indices of highly-central Dutch printmakers and publishers in 1560, 1585, 1615, 1645, 1675, 1715"}
top_eiis <- function(dat, yr, nat, n = 10) {
  dat %>%
    filter(!is.na(eii), nationality == nat, year == yr) %>%
    filter(min_rank(desc(total_degree_norm)) <= n) %>%
...
(ref:blooteling-rupert) Abraham Blooteling, *Portrait of Rupert, Prince of the Rhine*, 1673. Engraving, 39 x 29 cm. Rijksmuseum, Amsterdam.

```{r blooteling-rupert, fig.margin = TRUE, fig.cap="(ref:blooteling-rupert)"}
